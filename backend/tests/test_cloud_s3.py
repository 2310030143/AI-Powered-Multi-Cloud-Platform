...
def test_connect_with_request_body(client, auth_headers, monkeypatch):
    monkeypatch.setattr(
        "app.api.v1.endpoints.cloud_s3.S3StorageService", FakeS3Service
    )
...
def test_connect_without_credentials_returns_400(client, auth_headers):
    # No body and no S3_* env vars configured in the test environment
    response = client.post("/api/v1/cloud/s3/connect", headers=auth_headers)
